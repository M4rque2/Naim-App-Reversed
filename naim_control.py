...
def cmd_discover(args):
    ...
    # Phase 3: Verify/enrich each candidate via GET /system on port 15081
    # Legacy devices (e.g. SuperUniti) won't have port 15081 but are still
    # valid Naim devices if found via SSDP with a Naim manufacturer string.
    verified = []
    for ip, info in devices.items():
        ...
        elif info.get("source") in ("ssdp", "mdns"):
            # Legacy Naim device: no port 15081, but confirmed via SSDP/mDNS
            info["model"] = info.get("modelName", "Unknown")
    ...
    print(f"Found {len(verified)} Naim device(s):\n")
    for i, dev in enumerate(verified, 1):
        ...
        print(f"      Platform: {platform} ({'REST API :15081' if platform == 'leo' else 'UPnP/DLNA'})")
...
UPnP/DLNA (legacy devices like SuperUniti, NDS, NDX):
  %(prog)s --host 192.168.1.21 upnp-info
  %(prog)s --host 192.168.1.21 upnp-play
  %(prog)s --host 192.168.1.21 upnp-volume-get
  %(prog)s --host 192.168.1.21 upnp-volume-set --level 50
  %(prog)s --host 192.168.1.21 upnp-transport-info
        """,
    )
    ...
    # ── UPnP / DLNA (legacy devices) ──
    p = sub.add_parser("upnp-info", help="[UPnP] Fetch device description.xml")
    p.set_defaults(func=cmd_upnp_info, port=UPNP_DEFAULT_PORT)

    p = sub.add_parser("upnp-play", help="[UPnP] Start playback (AVTransport)")
    p.set_defaults(func=cmd_upnp_play, port=UPNP_DEFAULT_PORT)

    p = sub.add_parser("upnp-pause", help="[UPnP] Pause playback (AVTransport)")
    p.set_defaults(func=cmd_upnp_pause, port=UPNP_DEFAULT_PORT)

    p = sub.add_parser("upnp-stop", help="[UPnP] Stop playback (AVTransport)")
    p.set_defaults(func=cmd_upnp_stop, port=UPNP_DEFAULT_PORT)

    p = sub.add_parser("upnp-next", help="[UPnP] Next track (AVTransport)")
    p.set_defaults(func=cmd_upnp_next, port=UPNP_DEFAULT_PORT)

    p = sub.add_parser("upnp-prev", help="[UPnP] Previous track (AVTransport)")
    p.set_defaults(func=cmd_upnp_prev, port=UPNP_DEFAULT_PORT)

    p = sub.add_parser("upnp-seek", help="[UPnP] Seek to position (AVTransport)")
    p.add_argument("--target", required=True, help="Target time in HH:MM:SS format")
    p.set_defaults(func=cmd_upnp_seek, port=UPNP_DEFAULT_PORT)

    p = sub.add_parser("upnp-transport-info", help="[UPnP] Get transport state (AVTransport)")
    p.set_defaults(func=cmd_upnp_transport_info, port=UPNP_DEFAULT_PORT)

    p = sub.add_parser("upnp-position-info", help="[UPnP] Get current position (AVTransport)")
    p.set_defaults(func=cmd_upnp_position_info, port=UPNP_DEFAULT_PORT)

    p = sub.add_parser("upnp-media-info", help="[UPnP] Get media info (AVTransport)")
    p.set_defaults(func=cmd_upnp_media_info, port=UPNP_DEFAULT_PORT)

    p = sub.add_parser("upnp-volume-get", help="[UPnP] Get volume (RenderingControl)")
    p.set_defaults(func=cmd_upnp_volume_get, port=UPNP_DEFAULT_PORT)

    p = sub.add_parser("upnp-volume-set", help="[UPnP] Set volume (RenderingControl)")
    p.add_argument("--level", type=int, required=True, help="Volume level")
    p.set_defaults(func=cmd_upnp_volume_set, port=UPNP_DEFAULT_PORT)

    p = sub.add_parser("upnp-mute", help="[UPnP] Mute (RenderingControl)")
    p.set_defaults(func=cmd_upnp_mute, port=UPNP_DEFAULT_PORT)

    p = sub.add_parser("upnp-unmute", help="[UPnP] Unmute (RenderingControl)")
    p.set_defaults(func=cmd_upnp_unmute, port=UPNP_DEFAULT_PORT)

    p = sub.add_parser("upnp-mute-get", help="[UPnP] Get mute state (RenderingControl)")
    p.set_defaults(func=cmd_upnp_mute_get, port=UPNP_DEFAULT_PORT)
